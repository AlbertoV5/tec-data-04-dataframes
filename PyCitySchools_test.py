...
the dataset with only grades from 9th grade of Thomas High School missing.

We will get the averages and sort the values and compare the results.

This test can be run with pytest in the working directory.
"""
...
def modified_df(complete_df: pd.DataFrame):
    ...
    criteria = (complete_df["grade"] == "9th") & (complete_df.index == "Thomas High School")
...
def get_parameters(df: pd.DataFrame):
    ...
    per_school_passing_overall = df.loc[
        (df[READING_SCORE] >= 70)
        & (df[MATH_SCORE] >= 70), [READING_SCORE, MATH_SCORE]
    ]
    # Get total
    per_school_total_students = df.groupby(SCHOOL_NAME)[STUDENT_NAME].count()
    # Get passing percentages
    per_school_passing_math_percentage = 100 * per_school_passing_math.groupby(
        SCHOOL_NAME).count() / per_school_total_students
    per_school_passing_reading_percentage = 100 * per_school_passing_reading.groupby(
        SCHOOL_NAME).count() / per_school_total_students
    per_school_passing_overall_percentage = 100 * per_school_passing_overall[
        READING_SCORE].groupby(SCHOOL_NAME).count() / per_school_total_students
    parameters = {"Average Math Score": per_school_math_average,
...
def test_school_spending(complete_parameters: dict, modified_parameters: dict, schools_df: pd.DataFrame):
    """
...
def test_school_sizes(complete_parameters: dict, modified_parameters: dict, schools_df: pd.DataFrame):
    """
...
def test_school_types(complete_parameters: dict, modified_parameters: dict, schools_df: pd.DataFrame):
    """
